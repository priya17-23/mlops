...
def styles():
    ...
    s.add(ParagraphStyle("AssignHead", fontName="Times-Bold", fontSize=14, leading=18, alignment=TA_CENTER, textColor=INK, spaceAfter=4))
    s.add(ParagraphStyle("TitleMain", fontName="Times-Bold", fontSize=16, leading=20, alignment=TA_CENTER, textColor=INK, spaceAfter=4))
    s.add(ParagraphStyle("TitleSub", fontName="Times-Roman", fontSize=12, leading=16, alignment=TA_CENTER, textColor=INK, spaceAfter=10))
    s.add(ParagraphStyle("MetaLabel", fontName="Times-Bold", fontSize=11, leading=14, textColor=INK))
    s.add(ParagraphStyle("MetaVal", fontName="Times-Roman", fontSize=11, leading=14, textColor=INK))
...
def build_pdf():
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=16 * mm,
        bottomMargin=18 * mm,
        title="Assignment 1: Data Engineering – Introduction to Apache Pig",
        author="Priya N Singh",
    )

    story = []
    story.append(Paragraph("ASSIGNMENT 1: DATA ENGINEERING", st["AssignHead"]))
    story.append(Paragraph("Introduction to Apache Pig", st["TitleMain"]))
    story.append(Paragraph("Scenario: E-commerce Product Review Analysis", st["TitleSub"]))
    meta = [
        [
            Paragraph("Student Name", st["MetaLabel"]),
            Paragraph("Priya N Singh", st["MetaVal"]),
            Paragraph("USN / Roll No.", st["MetaLabel"]),
            Paragraph("4GW23CIO42", st["MetaVal"]),
        ]
    ]
    mt = Table(meta, colWidths=[1.4 * inch, 2.0 * inch, 1.5 * inch, 1.4 * inch])
    mt.setStyle(
        TableStyle(
            [
                ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
